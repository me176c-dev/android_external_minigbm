...
bool CrosGralloc1::Init()
{
	...
	if (strcmp(SURFACEFLINGER_PATH, getprogname()) ? driver->init() : driver->init_master()) {
			      ...
			      struct hw_device_t **device)
{
	...
	if (pCrosGralloc1 != NULL) {
		if (!strcmp(name, GRALLOC_HARDWARE_FB0)) {
			return cros_gralloc_open_framebuffer(&pCrosGralloc1->fb, pCrosGralloc1->driver->get_fd(),
						mod, &pCrosGralloc1->common, device);
		}
		*device = &pCrosGralloc1->common;
		return 0;
	} else if (!strcmp(name, GRALLOC_HARDWARE_FB0)) {
		ALOGE("Cannot open framebuffer with uninitalized gralloc");
		return -EINVAL;
	} else
